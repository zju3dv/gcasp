...
def compute_3d_iou_new(RT_1, RT_2, scales_1, scales_2, handle_visibility, class_name_1, class_name_2):
    ...
    def asymmetric_3d_iou(RT_1, RT_2, scales_1, scales_2):
        ...
        # gtbbox = gt_bbx3d[gt_ind]
        # predbbox = pred_bbx3d[ind]
        # lines = [[0, 1], [1, 3], [2, 3], [0, 2],
        #         [4, 5], [5, 7], [6, 7], [4, 6],
        #         [0, 4], [1, 5], [2, 6], [3, 7]]
        # gt_b = open3d.geometry.LineSet()
        # pred_b = open3d.geometry.LineSet()
        # gt_b.points = open3d.utility.Vector3dVector(np.array(gtbbox).T)
        # gt_b.lines = open3d.utility.Vector2iVector(lines)
        # pred_b.points = open3d.utility.Vector3dVector(np.array(predbbox).T)
        # pred_b.lines = open3d.utility.Vector2iVector(lines)
        # open3d.visualization.draw_geometries([pred_b, gt_b])
...
def compute_3d_matches(gt_class_ids, gt_RTs, gt_scales, gt_handle_visibility, synset_names,
    ...
    # print(pred_class_ids, gt_class_ids)
        ...
        for i in range(len(pred_boxes)):
            ...
            for j in sorted_ixs:
                ...
                if iou > iou_thres:
                    ...
                    pred_matches[s, i] = j
                    break
    # print(gt_matches, pred_matches, overlaps, indices)
...
def compute_degree_cm_mAP(final_results, result_lists, synset_names, log_dir, degree_thresholds=[360], shift_thresholds=[100], iou_3d_thresholds=[0.1], iou_pose_thres=0.1, use_matches_for_pose=False,     
    to_draw = ['mug']):
    ...
    pose_errors_all = [[] for i in range(num_classes)]
    errors_files = [[] for i in range(num_classes)]
    ...
    for progress, result in enumerate(final_results):
        ...
        for cls_id in range(1, num_classes):
            ...
            if(class_name in to_draw and RT_overlaps.shape[0]>0):
                # if (np.all(np.isnan(RT_overlaps)==False)):
                    for i in range(RT_overlaps.shape[0]):
                        pred_match = pose_cls_pred_match[-1,-1,i].astype(int)
                        pose_errors_all[cls_id].append(RT_overlaps[i,pred_match,:])
                        errors_files[cls_id].append(result_lists[progress])
    ...
    for i in range(1, num_classes):
        pose_errors_all[i] = np.array(pose_errors_all[i])
    ...
    for cls_id in range(1, num_classes):
        ...
        if(class_name not in to_draw):
            continue
        print(class_name)
        result_cmd_f = open(os.path.join(log_dir, f"{class_name}_eval_results.txt"), 'w')
        
        result_cmd_f.write('3D IoU at 25: {:.1f}\n'.format(iou_aps[cls_id, iou_thres_list.index(0.25)] * 100))
        result_cmd_f.write('3D IoU at 50: {:.1f}\n'.format(iou_aps[cls_id, iou_thres_list.index(0.5)] * 100))
        result_cmd_f.write('3D IoU at 75: {:.1f}\n'.format(iou_aps[cls_id, iou_thres_list.index(0.75)] * 100))
        result_cmd_f.write('1 degree, 1cm: {:.1f}\n'.format(pose_aps[cls_id, degree_thres_list.index(1),shift_thres_list.index(1)] * 100))
        result_cmd_f.write('1 degree, 1cm: {:.1f}\n'.format(pose_aps[cls_id, degree_thres_list.index(1),shift_thres_list.index(1)] * 100))
        result_cmd_f.write('5 degree, 2cm: {:.1f}\n'.format(pose_aps[cls_id, degree_thres_list.index(5),shift_thres_list.index(2)] * 100))
        result_cmd_f.write('5 degree, 5cm: {:.1f}\n'.format(pose_aps[cls_id, degree_thres_list.index(5),shift_thres_list.index(5)] * 100))
        result_cmd_f.write('10 degree, 2cm: {:.1f}\n'.format(pose_aps[cls_id, degree_thres_list.index(10),shift_thres_list.index(2)] * 100))
        result_cmd_f.write('10 degree, 5cm: {:.1f}\n'.format(pose_aps[cls_id, degree_thres_list.index(10),shift_thres_list.index(5)] * 100))
        result_cmd_f.write('10 degree, 10cm: {:.1f}\n'.format(pose_aps[cls_id, degree_thres_list.index(10),shift_thres_list.index(10)] * 100))
        result_cmd_f.write('15 degree, 5cm: {:.1f}\n'.format(pose_aps[cls_id, degree_thres_list.index(15),shift_thres_list.index(5)] * 100))
        result_cmd_f.write('15 degree, 10cm: {:.1f}\n'.format(pose_aps[cls_id, degree_thres_list.index(15),shift_thres_list.index(10)] * 100))
        
        print('3D IoU at 25: {:.1f}'.format(iou_aps[cls_id, iou_thres_list.index(0.25)] * 100))
        print('3D IoU at 50: {:.1f}'.format(iou_aps[cls_id, iou_thres_list.index(0.5)] * 100))
        print('3D IoU at 75: {:.1f}'.format(iou_aps[cls_id, iou_thres_list.index(0.75)] * 100))
        print('1 degree, 1cm: {:.1f}'.format(pose_aps[cls_id, degree_thres_list.index(1),shift_thres_list.index(1)] * 100))
        print('1 degree, 1cm: {:.1f}'.format(pose_aps[cls_id, degree_thres_list.index(1),shift_thres_list.index(1)] * 100))
        print('5 degree, 2cm: {:.1f}'.format(pose_aps[cls_id, degree_thres_list.index(5),shift_thres_list.index(2)] * 100))
        print('5 degree, 5cm: {:.1f}'.format(pose_aps[cls_id, degree_thres_list.index(5),shift_thres_list.index(5)] * 100))
        print('10 degree, 2cm: {:.1f}'.format(pose_aps[cls_id, degree_thres_list.index(10),shift_thres_list.index(2)] * 100))
        print('10 degree, 5cm: {:.1f}'.format(pose_aps[cls_id, degree_thres_list.index(10),shift_thres_list.index(5)] * 100))
        print('10 degree, 10cm: {:.1f}'.format(pose_aps[cls_id, degree_thres_list.index(10),shift_thres_list.index(10)] * 100))
        print('15 degree, 5cm: {:.1f}'.format(pose_aps[cls_id, degree_thres_list.index(15),shift_thres_list.index(5)] * 100))
        print('15 degree, 10cm: {:.1f}'.format(pose_aps[cls_id, degree_thres_list.index(15),shift_thres_list.index(10)] * 100))
        
        err = np.mean(pose_errors_all[cls_id], axis=0)
        ind = np.argsort(pose_errors_all[cls_id], axis=0).astype(int)
        result_f = open(os.path.join(log_dir,class_name+'_err.txt'), 'w')
        for i in range(pose_errors_all[cls_id].shape[0]):
            result_f.write(str(pose_errors_all[cls_id][ind[i][0]])+" "+errors_files[cls_id][ind[i][0]]+"\n")
        result_f.write("trans\n")
        for i in range(pose_errors_all[cls_id].shape[0]):
            result_f.write(str(pose_errors_all[cls_id][ind[i][1]])+" "+errors_files[cls_id][ind[i][1]]+"\n")

        print('Rot err {:.2f} degree, trans err: {:.2f} cm'.format(err[0], err[1]))
...
def draw(img, imgpts, axes, color):
    ...
    for i, j in zip([0, 1, 2, 3],[1, 3, 0, 2]):
        img = cv2.line(img, tuple(imgpts[i]), tuple(imgpts[j]), color, 2)
...
def draw_detections(image, save_dir, data_name, image_id, intrinsics, synset_names, draw_rgb_coord,
                    ...
                    draw_gt=False, draw_pred=True, draw_tag=False):
    ...
    if draw_gt:
        ...
        if gt_RTs is not None:
            for ind, RT in enumerate(gt_RTs):
                ...
                gt_bbx3d.append(transformed_bbox_3d)
    ...
    if draw_pred:
        ...
        if gt_class_ids is not None and False:
        ...
        for ind in range(num_pred_instances):
            RT = pred_RTs[ind]
            # print(ind, num_pred_instances, RT)
            cls_id = pred_class_ids[ind]
            
            if gt_class_ids is not None and False:## if gt exists, skip instances that fail to match
            ...
            transformed_bbox_3d = transform_coordinates_3d(bbox_3d, RT)
            pred_bbx3d.append(transformed_bbox_3d)
            projected_bbox = calculate_2d_projections(transformed_bbox_3d, intrinsics)
            draw_image_bbox = draw(draw_image_bbox, projected_bbox, projected_axes, (0, 200, 255))
        ...
        if draw_tag:
            if gt_class_ids is not None: ## if gt exists, draw rotation and translation error
                for ind in range(num_pred_instances):
                    ...
                    gt_RT = gt_RTs[gt_ind]
                    cls_id = pred_class_ids[ind]
                    degree, cm = compute_RT_degree_cm_symmetry(RT, gt_RT, cls_id, 1, synset_names)
                    if(np.isnan(degree)):
                        continue
                    ...
                    # if(np.array_equal(gt_RT[3, :], RT[3, :])):
                    #     continue
...
def align(class_ids, masks, coords, depth, intrinsics, synset_names, image_path, save_path=None, if_norm=False, with_scale=True, verbose=False):
    ...
    for i in range(num_instances):
        ...
        # pcd = open3d.geometry.PointCloud(open3d.utility.Vector3dVector(pts))
        
        coord_pts = coord[idxs[0], idxs[1], :] - 0.5
        
        # pcd2 = open3d.geometry.PointCloud(open3d.utility.Vector3dVector(coord_pts))
        ...
        try:
            ...
            if save_path is not None:
                ...
                # pcd2 = open3d.geometry.PointCloud(open3d.utility.Vector3dVector(coord_pts_rotated*1000))
