...
class NOCSDataset(utils.Dataset):
    ...
    def load_camera_scenes(self, dataset_dir, if_calculate_mean=False):
        ...
        # image_num = 3591
        # image_ids = range(image_num, image_num+1)
        ...
        for i in tqdm(image_ids):
            ...
            with open(meta_path, 'r') as f:
                for line in f:
                    ...
                    cls_ids.append(line_info[2])
                    inst_ids.append(line_info[3])
    ...
    def load_inst_info(self, image_id):
        info = self.image_info[image_id]

        return info['inst_num'], info['cls_num']
    ...
    def load_all_objs(self, obj_dir):
        def pc_normalize(pc):
            centroid = (np.amax(pc, axis=-2, keepdims=True) + np.amin(pc, axis=-2, keepdims=True))/2
            print(np.amax(pc, axis=-2, keepdims=True), np.amin(pc, axis=-2, keepdims=True))
            pc = pc - centroid
        ...
        self.obj_info = {}
        for mesh_file in os.scandir(obj_dir):
            if('norm.obj' not in mesh_file.name):
                continue
            vertices, faces = utils.load_mesh(mesh_file, is_normalized=True)
            _, scale, center = pc_normalize(vertices)
            self.obj_info[mesh_file.name.split('.')[0]] = [scale, center]
    ...
    def process_data(self, mask_im, coord_map, inst_dict, meta_path, load_RT=False):
        ...
        return masks, coords, class_ids, scales, origin_ids
    ...
    def load_mask(self, image_id):
        ...
        if info["source"] in ["CAMERA", "Real"]:
            ...
            masks, coords, class_ids, scales, origin_ids = self.process_data(mask_im, coord_map, inst_dict, meta_path)
        ...
        return masks, coords, class_ids, scales, domain_label, origin_ids
