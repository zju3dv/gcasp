...
class Trainer(DualSDFTrainer):
    ...
    def extract_points(self, feature):
        latent_codes_fine = feature.to(self.device)
        ...
        with torch.no_grad():
            sdf_fun, _ = self._get_render_sdfs(latent_codes_fine)
            points = extractor.extract(sdf_fun, coloridx=None)
        return points
    
    def compute_dists(self, feature, points):
        latent_codes_fine = feature.to(self.device)
        if not hasattr(self, 'extractor'):
            from toolbox.sdf2points import SDF2Points
            extractor = SDF2Points(self.device)
        ...
        dist = sdf_fun(points)
        prim_dist = prim_sdf_fun(points)
        return dist['dists'], prim_dist['dists']
