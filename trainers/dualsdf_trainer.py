...
class Trainer(BaseTrainer):
    ...
    def _get_lossfun(self, cfg):
        ...
        if cfg.type.lower() == 'clamped_l1':
            ...
            lossfun = onesided_l2
        elif cfg.type.lower() == 'twosided_l2':
            from models.lossfuns import twosided_l2
            lossfun = twosided_l2
        else:
    ...
    def extract_sdf(self, feature, voxel_size):
        latent_codes_fine = feature.to(self.device)
        if not hasattr(self, 'extractor'):
            from toolbox.sdf2points import SDF2Points
            extractor = SDF2Points(self.device)
        
        # Prepare DeepSDF for rendering
        self.eval()
        with torch.no_grad():
            sdf_fun, _ = self._get_render_sdfs(latent_codes_fine)
            points, dists = extractor.extract_sdf(sdf_fun, voxel_size, coloridx=None)
        return points, dists
